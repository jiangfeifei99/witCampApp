...
export function Init() {
  ...
  // captureObj.SetWatermarkText("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
}
...
export function StartCapture() {
  ...
  if (captureRet == emCaptureFailed) {
    ...
    var browserInfo = "查看控件是否被浏览器阻止，或通过浏览器设置中的加载项查看NiuniuCapture是否加载并正常运行";
    // eslint-disable-next-line no-undef
    $('#info').html('截图控件未能识别到，请按如下步骤检查:<br/>1. 确定您已经下载控件安装包并正常安装,请点此<a target="_blank" href="' + downloadUrl + '">安装</a>' +
      '<br/>2. ' + browserInfo +
      '<br/>3. 刷新页面或重新启动浏览器试下');
  }
}
export function SetWatermarkText(text) {
  captureObj.SetWatermarkText(text);
}
